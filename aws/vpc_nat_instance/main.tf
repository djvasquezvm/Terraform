resource "aws_instance" "ec2_basic_subnet1_nat" {
  ami           = var.ami_nat
  ...
  tags = {
    Name = var.tags_name_ec2_subnet1_nat
  }
}
...
resource "aws_route_table" "routingtable_subnet2" {
  vpc_id = aws_vpc.danielvasquezvpc.id
  route {
    cidr_block = var.all_ipv4
    instance_id = aws_instance.ec2_basic_subnet1_nat.id
  }

  tags = {
    Name = var.tags_name_route_table_subnet2
  }
}

resource "aws_route_table_association" "b" {
  subnet_id      = aws_subnet.subnet2.id
  route_table_id = aws_route_table.routingtable_subnet2.id
}
